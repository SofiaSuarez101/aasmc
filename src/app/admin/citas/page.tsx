...
const AdminCitasPage: React.FC = () => {
	...
	// Add state for available slots and selected date
		...
		try {
			...
			const selectedPsicologo =
				dropdownData.psicologos.find(
					(p: Psicologo) =>
						String(p.id) === String(cita?.id_psicologo)
				);
			...
			const notificationPayloads = [
				{
					id_psicologo: Number(cita?.id_psicologo),
					titulo: `Su cita del día ${citaDate} a las ${citaTime} con el estudiante ${
						selectedStudent?.nombre ?? ""
					} ha sido cancelada.`,
				},
				{
					id_estudiante: Number(cita?.id_estudiante),
					titulo: `Su cita del día ${citaDate} a las ${citaTime} con el psicólogo ${
	...
	const handleReschedule = async () => {
		...
		try {
			...
			// Send notifications to both psicologo and estudiante
			const selectedStudent = dropdownData.students.find(
				(s: Student) =>
			...
			const selectedPsicologo =
				dropdownData.psicologos.find(
					(p: Psicologo) =>
						String(p.id) === String(form.id_psicologo)
				);
			...
			const notificationPayloads = [
				...
				{
					id_psicologo: Number(form.id_psicologo),
					titulo: `Tu cita con el estudiante ${
						selectedStudent?.nombre ?? ""
					} ha sido reprogramada para el día ${citaDate} a las ${citaTime}.`,
				},
			];
	...
	const handleCreate = async () => {
		...
		try {
			...
			// Send notifications to both student and psychologist, including names and cita date/time
			const notificationPayloads = [
				{
					id_estudiante: Number(form.id_estudiante),
										...
										<TooltipProvider>
											<Tooltip>
												<TooltipTrigger asChild>
													<Link
														href={`/admin/citas/observaciones/${cita.id_cita}`}>
														<Button
															size="sm"
															variant="outline">
															Observaciones
